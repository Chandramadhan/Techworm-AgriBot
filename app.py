...
K.clear_session()  # <-- Add this

model = Sequential()
model.add(Dense(128, activation='relu'))
model.add(Dense(38, activation='softmax', name=f"output_layer_{np.random.randint(1000)}"))
# AI Tools
...
model.add(Dense(38, activation='softmax', name=f"output_layer_{str(np.random.randint(1000))}"))

 # For 38 classes
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])

# Custom label map based on your dataset
label_map = {
